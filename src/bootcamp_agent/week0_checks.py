...
def _w01_e2(locate: object) -> str | None:
    ...
    # happened the first time this shipped. So the call runs on a daemon thread
    # with a deadline, and a thread still alive after it IS the finding.
    outcome: dict[str, object] = {}

    def _attempt() -> None:
        try:
            outcome["value"] = locate(Path(tempfile.gettempdir()))
        except Exception as error:  # noqa: BLE001
            outcome["error"] = error
    ...
    if "error" in outcome:
        error = outcome["error"]
        return f"raised on a directory with no pyproject above it: {type(error).__name__}: {error}"
