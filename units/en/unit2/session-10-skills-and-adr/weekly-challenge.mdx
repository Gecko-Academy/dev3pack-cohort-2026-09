...
| Counts toward session 10? | no. Your session marks come from `ch10-e1` and `ch10-e2` |
...
Put `MY_STORE`, your `plan_purchase` and the `bonus(...)` cell in your session 10
notebook, under the exercises, with the score showing.

```bash
uv run bootcamp submit ch10 --github <your-github-name> --push
```

It does not change your session marks. It is there so the person reviewing sees
how far you took it.
