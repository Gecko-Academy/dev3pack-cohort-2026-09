...
You read the seam: a Protocol, four clients, and one `get_client` that turns four configuration fields into one of them.
You ran the same prompt on two lanes and watched the fake agree with itself twice while a real model did not.
...
The failure handled: a timeout. `answer_with_timeout` turns a provider that never answers into an `AgentResult` flagged for review, citing nothing, with the cause kept in the trace and out of the answer.
