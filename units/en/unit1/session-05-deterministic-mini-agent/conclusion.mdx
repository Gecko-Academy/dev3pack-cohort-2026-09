...
The repetition guard compares tool and arguments against the previous call, so a spinning loop reports `repeated_call` and not a budget that was fine.
The failure handled: a tool that answers twice and then starts refusing. The run ends in `tool_error` with a refusal that names the tool, and the two successful steps survive in the receipt.
`ch05-e1` proved the budget is visible in the trace at two budgets. `ch05-e2` ran your loop against one plan per exit, with tools that record what they were called with.
...
Homework: write the exit table for a loop you have built or used. An empty row is an unfinished loop. Then read `docs/guides/loop-engineering.md`.
